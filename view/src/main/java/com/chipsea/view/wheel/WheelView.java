...
public class WheelView extends TosGallery {
	 ...
	 * Shadow colors
	 */
	private final int[] SHADOWS_COLORS = { 0xE7FFFFFF, 0xE1FFFFFF, 0xC3FFFFFF,
			0xB0FFFFFF, 0x80FFFFFF, 0x60FFFFFF,0x30FFFFFF, 0x10FFFFFF};
	...
	public WheelView(Context context, AttributeSet attrs) {
		...
		TypedArray mTypedArray = context.obtainStyledAttributes(attrs,
				R.styleable.Wheel);
		int mLineColorMax = mTypedArray.getInt(R.styleable.Wheel_ShadowsColor,
				Color.WHITE);
		for (int i = 0; i < SHADOWS_COLORS.length; i++) {
			SHADOWS_COLORS[i] = SHADOWS_COLORS[i] & mLineColorMax;
		}
		mTypedArray.recycle();
		initialize(context);
	}
	...
	private void initialize(Context context) {
		...
		this.mTopShadow = new GradientDrawable(Orientation.TOP_BOTTOM,
				SHADOWS_COLORS);
		this.mBottomShadow = new GradientDrawable(Orientation.BOTTOM_TOP,
				SHADOWS_COLORS);
	...
	private void drawShadows(Canvas canvas) {
		...
		mBottomShadow.setBounds(0, getHeight() - height, getWidth(),
				getHeight());
