 ...
 */
public class TosGallery extends TosAbsSpinner implements
    ...
    public TosGallery(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.Gallery, defStyle, 0);
        ...
        int animationDuration = a.getInt(R.styleable.Gallery_animationDuration,
                -1);
        ...
        float unselectedAlpha = a.getFloat(R.styleable.Gallery_unselectedAlpha,
                0.5f);
        ...
        // The scroll bar size.
        mScrollBarSize = ViewConfiguration.get(context)
    ...
    protected boolean getChildStaticTransformation(View child, Transformation t) {
        ...
        Log(" getChildStaticTransformation   mSelectedPosition =  "
                + mSelectedPosition + "   mFirstPosition = " + mFirstPosition
                + "     mSelectedChild = " + mSelectedChild);
    ...
    void trackMotionScroll(int deltaX) {
        ...
        if (toLeft) {
            ...
            if (null != child
                    && child.getLeft() > (getWidth() - getStopFlingPosition())) {
                deltaX = 0;
    ...
    private float getStopFlingPosition() {
        if (isOrientationVertical()) {
            return (getHeight() - getPaddingTop() - getPaddingBottom())
                    * onStopFlingPosRatio() + getPaddingTop();
        }

        return (getWidth() - getPaddingLeft() - getPaddingRight())
                * onStopFlingPosRatio() + getPaddingLeft();
    }
        ...
        if (0 == mFirstPosition) {
            ...
            if (child.getLeft() >= 0) {
                ...
                if ((lastChild.getRight() - child.getLeft()) < (getRight() - getPaddingRight())) {
                    ...
                    scrollAmount = getWidth() - getPaddingRight()
                            - lastChild.getRight();
                }
            }
        }
        ...
        if (0 == mFirstPosition) {
            ...
            if (child.getTop() >= 0) {
                ...
                if ((lastChild.getBottom() - child.getTop()) < (getBottom() - getPaddingBottom())) {
                    ...
                    scrollAmount = getHeight() - getPaddingBottom()
                            - lastChild.getBottom();
                }
            }
        }
    ...
    void layout(int delta, boolean animate) {
        ...
        if (isSlotInCenter()) {
            int childrenWidth = getRight() - getLeft() - mSpinnerPadding.left
                    - mSpinnerPadding.right;
            selectedOffset = childrenLeft + (childrenWidth / 2)
                    - (sel.getWidth() / 2);
        }
    ...
    void layoutVertical(int delta, boolean animate) {
        ...
        if (isSlotInCenter()) {
            ...
            selectedOffset = childrenTop + (childrenHeight / 2)
                    - (sel.getHeight() / 2);
        }
    ...
    private void fillToGalleryTop() {
        ...
        while (curRightEdge > galleryTop && curPosition >= 0) {
            prevIterationView = makeAndAddViewVertical(curPosition, curPosition
                    - mSelectedPosition, curRightEdge, false);
    ...
    private View makeAndAddViewVertical(int position, int offset, int y,
                                        boolean fromTop) {
     ...
     *                building from left to right)?
     */
    private void setUpChildVertical(View child, int offset, int y,
                                    boolean fromTop) {
    ...
    private void updateSelectedItemMetadata() {
        ...
        Log(" updateSelectedItemMetadata   mSelectedPosition =  "
                + mSelectedPosition + "   mFirstPosition = " + mFirstPosition);
    ...
    public int scrollGalleryItems(boolean toLeft) {
        ...
        if (toLeft) {
            View child = getChildAt(getChildCount() - 1);
            offset = child.getRight() - this.getRight()
